...
✅ **局域网版本**（`lan-v1`，v0.1.12）：两台同局域网 Windows 电脑，串口设备远程映射，双向透传。已通过两台真机联调验证。
...
4. "波特率"填设备要求的值。
5. 点 **"连接"**。首次会弹 UAC，点"是"装驱动（只一次，以后不用）。连接成功后界面显示分配的虚拟口，如 `COM50（串口软件开此口）`。
6. 打开你的串口软件，选 **COM50**（界面提示的那个号），设相同波特率，即可收发数据。
...
├── CHANGELOG.md            更新日志
└── Directory.Build.props   全局版本号（当前 0.1.12）
```
